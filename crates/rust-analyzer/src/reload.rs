...
impl GlobalState {
    ...
    pub(crate) fn update_configuration(&mut self, config: Config) {
        ...
        if self.config.linked_or_discovered_projects() != old_config.linked_or_discovered_projects()
        {
            self.fetch_workspaces_queue
                .request_op("discovered projects changed".to_owned(), (None, true))
    ...
    pub(crate) fn current_status(&self) -> lsp_ext::ServerStatusParams {
        ...
        if !self.config.cargo_autoreload()
            ...
            && self.config.discover_command().is_none()
        {
    ...
    pub(crate) fn fetch_workspaces(
        ...
        self.task_pool.handle.spawn_with_sender(ThreadIntent::Worker, {
            ...
            let discover_command = self.config.discover_command();
                ...
                let mut workspaces = linked_projects
                    ...
                    .map(|project| match project {
                        ...
                        LinkedProject::InlineJsonProject(it) => {
                            let (Some(_command), Some(path)) = (&discover_command, &path) else {
                            ...
                            if let Some(build) = it.crate_by_buildfile(path) {
                                if is_quiescent {
...
pub(crate) fn should_refresh_for_change(path: &AbsPath, change_kind: ChangeKind) -> bool {
    ...
    if let "Cargo.toml" | "Cargo.lock" | "BUCK" | "TARGETS" = file_name {
        return true;
    }
