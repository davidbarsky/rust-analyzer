...
use crate::{AnchoredPath, Vfs, VfsPath, File};

/// A set of [`VfsPath`]s identified by [`FilePathInput`]s.
#[derive(Default, Clone, Eq, PartialEq)]
pub struct FileSet {
    files: FxHashMap<VfsPath, File>,
    paths: FxHashMap<File, VfsPath>,
}
...
impl FileSet {
    ...
    /// Insert the `file_path, path` pair into the set.
    ///
    /// # Note
    /// Multiple [`FilePathInput`] can be mapped to the same [`VfsPath`], and vice-versa.
    pub fn insert(&mut self, file_path: File, path: VfsPath) {
        self.files.insert(path.clone(), file_path);
        self.paths.insert(file_path, path);
    }
...
impl FileSetConfig {
    ...
    pub fn partition(&self, db: &dyn salsa::Database, vfs: &Vfs) -> Vec<FileSet> {
        let mut scratch_space = Vec::new();
        let mut res = vec![FileSet::default(); self.len()];
        for file_path in vfs.iter() {
            let path = file_path.path(db);
            let root = self.classify(&path, &mut scratch_space);
            res[root].insert(*file_path, path.clone());
        }
        res
    }
