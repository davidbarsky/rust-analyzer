...
///
/// *Most* different crates have a different root files (actually, pretty much all of them).
...
pub struct UniqueCrateData {
    root_file_id: FileId,
    disambiguator: Option<(CrateData, HashableCfgOptions)>,
}

#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct CrateDataGeneric<Id> {
    ...
    pub dependencies: Vec<DependencyGeneric<Id>>,
    pub origin: CrateOrigin,
    pub is_proc_macro: bool,
}

pub type CrateDataBuilder = CrateDataGeneric<CrateBuilderId>;
pub type CrateData = CrateDataGeneric<Crate>;
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct DependencyGeneric<Id> {
    pub crate_id: Id,
    pub name: CrateName,
    prelude: bool,
    sysroot: bool,
...
pub type DependencyBuilder = DependencyGeneric<CrateBuilderId>;
pub type Dependency = DependencyGeneric<Crate>;
...
pub struct Crate {
    #[return_ref]
    pub data: CrateData,
    /// Some crate data that is not needed for analysis, split to help incrementality.
    #[return_ref]
...
impl CrateGraphBuilder {
    ...
    pub fn set_in_db(self, db: &mut dyn RootQueryDb) -> CratesIdMap {
        ...
        let old_all_crates = db.all_crates().iter().copied().collect::<FxHashSet<_>>();
        ...
        if old_all_crates.len() != all_crates.len()
            || all_crates.iter().any(|krate| !old_all_crates.contains(krate))
        {
        ...
        fn go(
            ...
            let disambiguator = if visited_root_files.insert(krate.basic.root_file_id) {
                ...
                Some((crate_data.clone(), krate.cfg_options.to_hashable()))
            };
