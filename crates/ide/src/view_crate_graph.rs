...
use ide_db::{
    base_db::{Crate, CrateData, Dependency, ExtraCrateData, RootQueryDb, SourceDatabase},
...
struct DotCrateGraph<'db> {
    crates_to_render: FxHashMap<Crate, (&'db CrateData, &'db ExtraCrateData)>,
}

type Edge<'a> = (Crate, &'a Dependency);
